...
void testCamera_FaceMasking()
{
	...
	Face3DMM face_3dmm;
	face_3dmm.initialize(
		"face_reconstruction.ncnn.param",
		"face_reconstruction.ncnn.bin"
	);
	FaceRender face_render;
	face_render.initialize("X:\\checkpoints\\deep-3d-reconstruction\\cpp\\face_masking.bin");

	cv::VideoCapture capture("N:\\archive\\2024\\1126-video\\Obama\\input-16.mp4");
	...
	std::string path("X:\\project\\xhuman-cpp\\asset\\input-16-masking.mp4");
	std::string path_video_out_str = formatString("%s.avi", path.substr(0, path.find_last_of('.')).c_str());
	bool flag = writer.open(path_video_out_str.c_str(), cv::VideoWriter::fourcc('X', 'V', 'I', 'D'), 30, cv::Size(w, h), true);
	assert(flag == true);
	...
	while (capture.read(mat) == true)
	{
		...
		face_3dmm.inference(image, result_vector);
		FaceRenderResult result_render;
		// masking with texture
		//face_render.inference(result_vector[0], uv_texture, result_render);
		// masking without texture
		...
		int key = cv::waitKey(1);
		if (key == 'q') break;
		writer.write(result_source.image);
	}
